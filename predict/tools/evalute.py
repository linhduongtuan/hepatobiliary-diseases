...
def confidence_interval(cm, auc, sens, spec, coef=1.96):
    """
    说明：计算95%置信区间
    """
    auc_down = auc - coef * (auc * (1 - auc) / np.sum(cm)) ** 0.5
    auc_up = auc + coef * (auc * (1 - auc) / np.sum(cm)) ** 0.5

    sens_down = sens - coef * (sens * (1 - sens) / np.sum(cm, axis=1)[1]) ** 0.5
    sens_up = sens + coef * (sens * (1 - sens) / np.sum(cm, axis=1)[1]) ** 0.5

    spec_down = spec - coef * (spec * (1 - spec) / np.sum(cm, axis=1)[0]) ** 0.5
    spec_up = spec + coef * (spec * (1 - spec) / np.sum(cm, axis=1)[0]) ** 0.5

    return (auc_down, auc_up), (sens_down, sens_up), (spec_down, spec_up)
...
def evalute(epoch, epochs, model, model_name, loader, dataset, criterion, threshold=0.5, write_log='', _type='Val',
    ...
    with torch.no_grad():
        ...
        auc_confidence, sens_confidence, spec_confidence = confidence_interval(cm, auc, sens, spec)
...
def mutil_attention_evalute(epoch, epochs, model, model_name, loader, dataset, criterion, threshold=0.5, write_log='', _type='Val',
                            draw_roc=False, title=None, name=None, save_pred=False, save_scores=False):
    begin = time.time()
    if write_log != '':
        logs = open(write_log, 'a')
